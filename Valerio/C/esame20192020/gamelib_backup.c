...
static void avanza();
static void azzeramento_scavatrici();
...
void FirstMenu(bool * wannaPlay) {
  ...
  do {
    ...
    safeScanf(&choice);
    switch (choice) {
      case '1': crea_cunicoli(&mappa); break;
      case '2': gioca(&mappa); break;
      case '3': termina_gioco(ragione_deallocazione, &mappa); break;
      default: warn('1');
    }
  } while(choice != '3');
  char playChoice;
...
// ~/   | Cancella i cunicoli
void freeMap(bool * map, enumFree whichFree) {
...
static US probabilita(enumProbabilita tipo, bool abbattimento) {
  ...
  switch (tipo) {
    ...
    case cSuperColpoAssassino: return rand() % 41 + 30; break;
  }
}
...
static void aggira() {
  ...
  *cunicoli[currentPlayer->eFamiglia] = currentPlayer->posizione;
  ...
  // directions[0] = "\x1b[93mdavanti\x1b[0m a te";
  // directions[1] = "alla tua \x1b[93mdestra\x1b[0m";
  // directions[2] = "alla tua \x1b[93msinistra\x1b[0m";
  // printf(
  //   "\nConsumando \x1b[91m1 unità\x1b[0m di melassa hai abbattuto la parete %s spostandoti in una nuova caverna\n",
...
static void jsPrintAction(enumMossa action, char nextDirection) {
  ...
  string[i + 4] = '\0';
  sprintf(command, "clear; echo '%s' | ./printAction.js", string);
  system(command);
  clr;
}
