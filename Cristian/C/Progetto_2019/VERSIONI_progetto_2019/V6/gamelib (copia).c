...
//RIGA 1647. GUARDA COMMENTI E CHIEDI.
        ...
        if (scelta_personaggio == 1) //Ho scelto Arvais
          {
            ...
            if (numero_caverne_Arvais == 0)
            ...
            else
              {
                ...
                printf("QUANTE CAVERNE VUOI INSERIRE?\n\nNUMERO CAVERNE DA INSERIRE: ");
                green();
                while (scanf("%hd",&numero_caverne) != 1 || numero_caverne < 1 || numero_caverne > 100)
                  {
                    blu();
                    printf("SCELTA NON CORRETTA! REINSERISCI IL VALORE: ");
                    while (getchar() != '\n');
...
int aggira (struct Caverna *pAggira_caverna, struct Scavatrice *pPersonaggio, int i)
  {
    ...
    /* DATO CHE pAggira_caverna ha ancora lo stesso indirizzo di pPrimo chiamato dalla funzione non lo devo aggiornare.
       Stessa cosa per i puntatori di direzione che appartengono allo stesso indirizzo.
       Per pAggira invece come faccio a far si che esista anche dopo essere uscito da questa funzione?
    */
...
int gioca (void)
  {
                            ...
                            for (int a = 0; a < numero_caverne_Arvais; a++)
                              {
                                if(pPrimo_Arvais_caverna == NULL) //Non sono presenti nodi
                                ...
                                else
                                  {
                                    ...
                                    printf("RIGA 1550. pPrimo_Arvais_caverna: %p\n", pPrimo_Arvais_caverna);
                                    printf("RIGA 1550. pPrimo_Arvais_caverna -> sinistra: %p\n", pPrimo_Arvais_caverna -> sinistra);
                                    printf("RIGA 1550. pPrimo_Arvais_caverna -> avanti: %p\n"  , pPrimo_Arvais_caverna -> avanti);
                                    printf("RIGA 1550. pPrimo_Arvais_caverna -> destra: %p\n"  , pPrimo_Arvais_caverna -> destra);
                                  }
                              }
                          }
                        else
                              ...
                              for (int a = 0; a < numero_caverne_Hartornen; a++)
                                {
                                  if(pPrimo_Hartornen_caverna == NULL) //Non sono presenti nodi
                                  ...
                                  else
                                    {
                                      ...
                                      printf("RIGA 1730. pPrimo_Hartornen_caverna: %p\n", pPrimo_Hartornen_caverna);
                                      printf("RIGA 1730. pPrimo_Hartornen_caverna -> sinistra: %p\n", pPrimo_Hartornen_caverna -> sinistra);
                                      printf("RIGA 1730. pPrimo_Hartornen_caverna -> avanti: %p\n"  , pPrimo_Hartornen_caverna -> avanti);
                                      printf("RIGA 1730. pPrimo_Hartornen_caverna -> destra: %p\n"  , pPrimo_Hartornen_caverna -> destra);
                                    }
                                }
                            }
                          else
    ...
    printf("RIGA 1765. pPrimo_Arvais_caverna: %p\n", pPrimo_Arvais_caverna);
    printf("RIGA 1765. pPrimo_Arvais_caverna -> sinistra: %p\n", pPrimo_Arvais_caverna -> sinistra);
    printf("RIGA 1765. pPrimo_Arvais_caverna -> avanti: %p\n"  , pPrimo_Arvais_caverna -> avanti);
    printf("RIGA 1765. pPrimo_Arvais_caverna -> destra: %p\n"  , pPrimo_Arvais_caverna -> destra);

    //DOPO ELIMINA
    int b = 0;

    while (pPrimo_Arvais_caverna -> sinistra != NULL || pPrimo_Arvais_caverna -> avanti != NULL || pPrimo_Arvais_caverna -> destra != NULL)
      {
        b++;
        struct Caverna *temp_3;
        if (pPrimo_Arvais_caverna -> sinistra != NULL)
            temp_3 = pPrimo_Arvais_caverna -> sinistra;
        else if (pPrimo_Arvais_caverna -> avanti != NULL)
            temp_3 = pPrimo_Arvais_caverna -> avanti;
        else if (pPrimo_Arvais_caverna -> destra != NULL)
            temp_3 = pPrimo_Arvais_caverna -> destra;

        printf ("WHILE 1: GIRO NUMERO° [%d]\n",++b);
        free (pPrimo_Arvais_caverna); //Elimino l'ultima caverna creata
        pPrimo_Arvais_caverna = temp_3; //Aggiorno il puntatore di testa alla caverna successiva
      }

    printf("RIGA 1767. pPrimo_Arvais_caverna: %p\n", pPrimo_Arvais_caverna);
    printf("RIGA 1767. pPrimo_Arvais_caverna -> sinistra: %p\n", pPrimo_Arvais_caverna -> sinistra);
    printf("RIGA 1767. pPrimo_Arvais_caverna -> avanti: %p\n"  , pPrimo_Arvais_caverna -> avanti);
    printf("RIGA 1767. pPrimo_Arvais_caverna -> destra: %p\n"  , pPrimo_Arvais_caverna -> destra);
    ...
    //DOPO ELIMINA
    b = 0;
    while (pPrimo_Hartornen_caverna -> sinistra != NULL || pPrimo_Hartornen_caverna -> avanti != NULL || pPrimo_Hartornen_caverna -> destra != NULL)
      {
        struct Caverna *temp_4;
        if (pPrimo_Hartornen_caverna -> sinistra != NULL)
            temp_4 = pPrimo_Hartornen_caverna -> sinistra;
        else if (pPrimo_Hartornen_caverna -> avanti != NULL)
            temp_4 = pPrimo_Hartornen_caverna -> avanti;
        else if (pPrimo_Hartornen_caverna -> destra != NULL)
            temp_4 = pPrimo_Hartornen_caverna -> destra;

        printf ("WHILE 2: GIRO NUMERO° [%d]\n",++b);
        free (pPrimo_Hartornen_caverna); //Elimino l'ultima caverna creata
        pPrimo_Hartornen_caverna = temp_4; //Aggiorno il puntatore di testa alla caverna successiva
      }
