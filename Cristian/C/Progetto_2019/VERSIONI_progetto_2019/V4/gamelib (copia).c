        ...
        if (flag_possibile_uscita_Arvais == 0 && i%2 == 0)
          ...
          if (flag_morte_Hartornen == 1 || flag_morte_Arvais == 1)
            ...
            if (pAvanza -> imprevisto == nessun_imprevisto)
              {
                blu(); printf("\n\n HAI CONTINUATO A ESPLORARE LA CAVERNA SENZA IMPREVISTI!\n");
                green();
                sleep(3);
              }
...
int aggira (struct Caverna *pAggira, struct Scavatrice *pPersonaggio, int i)
  {
    short int scelta_quantita_melassa = 0;
    short int scelta_tipo_imprevisto = 0;
    short int scelta_tipo_caverna = 0;
                   ...
                   switch (scelta_opzioni_gioca)
                     {
                       ...
                       case 3: //Aggira
                         if (flag_crollo_Arvais == 1)
                           {
                             aggira (primo_cunicolo_Arvais, pScavatrice_Arvais, i);

                             //AGGIORNAMENTO PUNTATORI
                             primo_cunicolo_Arvais -> avanti = pPrimo_Arvais_caverna; //Ho creato una nuova caverna per aggirare l'ostacolo
                             pPrimo_Arvais_caverna = primo_cunicolo_Arvais; //Aggiorno l'indirizzo di pPrimo_Arvais_caverna cosi' che punti nuovamente al primo elemnto della lista
                    ...
                    switch (scelta_opzioni_gioca)
                      {
                        ...
                        case 3: //Aggira
                          if (flag_crollo_Hartornen == 1)
                            {
                              aggira (primo_cunicolo_Hartornen, pScavatrice_Hartornen, i);
