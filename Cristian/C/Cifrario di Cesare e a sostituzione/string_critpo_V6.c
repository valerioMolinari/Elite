...
* - NON FUNZIONA QUANDO INSERISCO UNA LETTERA AL POSTO DELLA CHIAVE (RIGA 84)<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<
* SUL CIFRARIO A SOSTITUZIONE:
* - Trovare il metodo per separare la fine a buon esito con la reimmissione di un'altro alfabeto perchè il precedente inserito è erreato <<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<
* - Ho inserito la parte che mi controlla che sull'alfabeto inserito non vi sia la ripetizione di più caratteri uguali

RIGHE 167-173: Da quando ho invertito else_if con else non funziona il PROGRAMMA
insieme al vecchio else_if ci stava solo un continue
mentre la condizione del vecchio else era: alfabeto_sostituzione[i] >= 97 && alfabeto_sostituzione[i] <= 122
...
int main () {
  ...
  int  chiave, metodo, b = 0, caratteri_diversi = 0, ripetuto = 0;
              ...
              if (strlen(alfabeto_sostituzione)==26) //se la lunghezza dell'alfabeto sostitutivo è giusta
                {
                  ...
                  for (int i=0; alfabeto_sostituzione[i] != '\0'; i++)
                      ...
                      if (alfabeto_sostituzione[i] >= 65 && alfabeto_sostituzione[i] <= 90) //se compaiono lettere MAIUSCOLE le riporto in MINUSCOLE
                        {
                          ...
                          printf("RIGA 163. alfabeto_sostituzione[%d]: %c\n",i,alfabeto_sostituzione[i]);//<<<<<<<<<<<<<<<<<<<<<<<<TOGLILO DOPO<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<
                          i--;//l'ho messo così che, dopo averla trasformata in minuscola, possa arrivare al codice di verifica dei caratteri doppi;
                          continue;
                        }
                      else if (alfabeto_sostituzione[i] < 97 || alfabeto_sostituzione[i] > 122) //se non appartiene all'alfabeto
                        {
                          printf("RIGA 169. alfabeto_sostituzione[%d]: %c\n",i,alfabeto_sostituzione[i]);//<<<<<<<<<<<<<<<<<<<<<<<<TOGLILO DOPO<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<
                          printf("L'alfabeto inserito contiene caratteri diversi da quelli dell'alfabeto inglese\n");
                          break;
                        }
                      else //se è una lettera minuscola
                        {
                          printf("RIGA 175. alfabeto_sostituzione[%d]: %c\n",i,alfabeto_sostituzione[i]);//<<<<<<<<<<<<<<<<<<<<<<<<TOGLILO DOPO<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<
                          for(b=0; ((alfabeto_sostituzione[i] != alfabeto_sostituzione[b]) &&  alfabeto_sostituzione[b] != '\0' ) || b==i; b++)//verifico se vi sono caratteri ripetuti
                            {
                    ...
                    }//CI ARRIVO DALLA RIGA 170(SE NON APPARTIENE ALL'ALFABETO), 191(SE L'ALFABETO CONTIENE CARATTERI RIPETUTI), O SE E' TUTTO GIUSTO
            ...
            else
                {
                  ...
                  caratteri_diversi = 26;//se i due alfabeti hanno lunghezza diversa richiedo di inserire nuovamente l'alfabeto<<<<<<<<<<<<<<<<<NON VA PIU BENEEE<<<<<<<<<<<<<<<<<<<<<<<<<<<<<
                }

              //printf ("\nOperazione conclusa.\n"); //forse va inserito nella parentesi sopra else
          } while((b==25 && caratteri_diversi < 25) || (strlen(alfabeto_sostituzione)!=26));
        }
