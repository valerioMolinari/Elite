...
int main () {
  ...
  int  chiave, metodo, b = 0, alfabeto_sbagliato=0, alfabeto_giusto=0, caratteri_uguali=0;
  ...
  if (metodo == 1)  //SE HO SCELTO CIFRARIO DI CESARE
    {
      ...
      else if(metodo == 2) //SE HO SCELTO CIFRARIO A SOSTITUZIONE
        {
          do
          {
            do
             {
                ...
                printf("RIGA 165. alfabeto_sostituzione: %s\n", alfabeto_sostituzione);
                    ...
                    for (int i=0; alfabeto_sostituzione[i] != '\0'; i++)
                      {
                        if (alfabeto_sostituzione[i] >= 65 && alfabeto_sostituzione[i] <= 90) //se compaiono lettere MAIUSCOLE le riporto in MINUSCOLE
                          ...
                          {
                            printf("RIGA 184. alfabeto_sostituzione[%d]: %c\n",i,alfabeto_sostituzione[i]);//<<<<<<<<<<<<<<<<<<<<<<<<TOGLILO DOPO<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<
            ...
            printf("RIGA 194. alfabeto_giusto: %d\n",alfabeto_giusto);//<<<<<<<<<<<<<<<<<<<<<<<<TOGLILO DOPO<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<
            ...
            if (alfabeto_giusto==26)
              {
                ...
                for (int i=0; alfabeto_sostituzione[i] != '\0'; i++)//verifico se vi sono caratteri ripetuti
                  {
                    printf("RIGA 201. alfabeto_sostituzione i[%d]: %c\n",i,alfabeto_sostituzione[i]);
                    for(b=0; alfabeto_sostituzione[b] != '\0'; b++)
                      {
                        if (alfabeto_sostituzione[i] == alfabeto_sostituzione[b])
                          {
                            printf("RIGA 206. alfabeto_sostituzione b[%d]: %c\n",b,alfabeto_sostituzione[b]);
                            caratteri_uguali++;
                            printf("RIGA 208. alfabeto_sostituzione[%d]: %d\n",i,b);
                            printf("RIGA 210. caratteri_uguali: %d\n", caratteri_uguali);
                    ...
                    b = 0;
                    caratteri_uguali = 0;
                  }//CI ARRIVO DA RIGA 220(SE TROVO CARATTERI UGUALI), O SE TUTTO E' CORRETTO

                printf("RIGA 225. caratteri_uguali: %d\n", caratteri_uguali);
                if(caratteri_uguali == 0)
                  {
                    printf("Forse l'alfabeto inserito è giusto\n");
                    //SCRIVI QUA IL CODICE DI SOSTITUZIONE CON BREAK FINALE;<<<<<<<<<<<<<<<<<<<<<<<< TOGLI DOPO <<<<<<<<<<<<<<<<<<<<<<<<
                    for (int i=0; stringa[i] != '\0'; i++)//sostituisco le lettere della stringa in base all'alfabeto cifrato
                      {
                        printf("RIGA 235. Ho avviato la conversione\n");
                        printf("RIGA 236. stringa PRIMA conversione i[%d]: %c\n",i,stringa[i]);
                        if (stringa[i] >= 65 && stringa[i] <= 90) //se compaiono lettere MAIUSCOLE le riporto in MINUSCOLE
                          {
                            stringa[i] = stringa[i] +32;
                            i--;
                            printf("\n\nRIGA 241. stringa: %s\n\n",stringa);//<<<<<<<<<<<<<<<< ELIMINA DOPO <<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<
                        ...
                        printf("RIGA 254. stringa DOPO conversione i[%d]: %c\n",i,stringa[i]);
                        switch (stringa[i])
                          {
                            case 'a':
                        ...
                        printf("RIGA 328. stringa_cifrata[%d]: %c\n",i,stringa_cifrata[i]);
                      }
                    printf("stringa: %s\n",stringa);
