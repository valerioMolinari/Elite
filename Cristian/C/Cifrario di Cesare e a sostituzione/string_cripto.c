...
* - Ho eliminato il goto del metodo e l'ho rimpiazzato con un do_while
***************************************************************************************************************************
* 15/11/2019 ----> string_critpo_V10.c
* - Ho messo la chiave di criptazione sottoforma di char cosi' da eliminare il problema del loop.
*   Per farlo ho dovuto modificare tutta la prima parte del cifrario di Cesare.
* - Ho eliminato i commenti inutilli
*
...
int main () {
  ...
  int  chiave_int=0, chiave_temp_int=0, chiave_errata=0, lunghezza_chiave=0, metodo, metodo_sbagliato=0, b=0, alfabeto_sbagliato=0, alfabeto_giusto=0, caratteri_uguali=0;
  ...
  char stringa [256];
  char chiave_char [10];
  //char chiave_sicurezza [10];
  char alfabeto_sostituzione [27];
  char stringa_cifrata [256];

printf("%s\n%s\n%s\n%s\n%s\n\n"," REGOLE PER LA SCRITTURA DEL MESSAGGIO DA CIFRARE:",
                                " - Il messaggio da criptare puo' essere di massimo 256 caratteri.",
                                " - Se supera questo limite il testo viene troncato.",
                                " - Puo' essere scritto in maiuscolo ma verra' riportato in minuscolo.",
                                " - Puo' contenere caratteri speciali o numeri ma non verranno cifrati.");
  ...
  if (metodo == 1)  //SE HO SCELTO CIFRARIO DI CESARE
    {
      printf("\n%s\n%s\n%s\n%s\n%s\n%s\n%s\n%s\n\n","FUNZIONAMENTO CIFRARIO DI CESARE:",
                                                    ...
                                                    "    Puo' essere scritto in maiuscolo ma verra' riportato in minuscolo.",
                                                    "    Puo' contenere caratteri speciali o numeri ma non verranno cifrati.",
                                                    " -> La chiave di cifratura deve:",
                                                    "    - essere un numero compreso fra 1 e 26.",
      ...
      do
        {
          ...
          fgets   (chiave_char, sizeof(chiave_char), stdin);
          getchar ();
          //strncpy (chiave_char, chiave_sicurezza, sizeof(chiave_char));
          chiave_char [sizeof(chiave_char)-1] = '\0';
          lunghezza_chiave = strlen(chiave_char)-1;

           for (int i=0; i<strlen(chiave_char)-1; i++)
            {
              if(chiave_char[i] >= 48 && chiave_char[i] <= 57)
               {
                    switch (chiave_char[i])
                      {
                        case '0':
                          chiave_temp_int = (chiave_char[i] -48) *pow(10,lunghezza_chiave-1);
                          break;
                        case '1':
                          chiave_temp_int = (chiave_char[i] -48) *pow(10,lunghezza_chiave-1);
                          break;
          ...
          if (chiave_int < 1 || chiave_int > 26 || chiave_errata == 1)
            {
              printf ("Chiave errata. Deve essere compresa fra 1 e 26.\n");
              chiave_errata = 1;
            }
        ...
        for (int i=0; stringa[i] != '\0'; i++)
          {
            ...
            {
              if (stringa[i] + chiave_int > 90) //se il carattere da criptare appartiene all'alfabeto ma, aggiunta la chiave,è maggiore della Z, riparto da A
              {
                stringa[i] = stringa[i] + chiave_int -26;
                printf ("%c",stringa[i] + 32);
                continue;
              }
              printf ("%c",stringa[i] +32 + chiave_int);
              continue;
            }
        ...
        {
          printf("\n%s\n%s\n%s\n%s\n%s\n%s\n%s\n%s\n%s\n%s\n\n","FUNZIONAMENTO CIFRARIO A SOSTITUZIONE:",
                                                                " -> Il messaggio da criptare puo' essere di massimo 256 caratteri.",
                                                                "    Se supera questo limite il testo viene troncato.",
                                                                "    Puo' essere scritto in maiuscolo ma verra' riportato in minuscolo.",
                                                                "    Puo' contenere caratteri speciali o numeri ma non verranno cifrati.",
                                                                " -> L'alfabeto di cifratura deve:",
                                                                "    - essere di tipo inglese (26 caratteri).",
                                                                "    - non contenere caratteri speciali o comunque diversi da quelli dell'alfabeto inglese.",
                                                                "    - non contenere dei caratteri doppi.",
                                                                "    - puo' essere scritto in maiuscolo ma verra' riportato in minuscolo.");
          do
          {
            do
             {
                    ...
                    for (int i=0; alfabeto_sostituzione[i] != '\0'; i++)
                      {
                        if (alfabeto_sostituzione[i] >= 65 && alfabeto_sostituzione[i] <= 90) //se compaiono lettere MAIUSCOLE le riporto in MINUSCOLE
                          ...
                          {
                            printf("\nL'alfabeto inserito contiene caratteri diversi da quelli dell'alfabeto inglese.\n");
              ...
              if (strlen(alfabeto_sostituzione)!=26)
                printf("\nL'alfabeto inserito e' troppo corto.\n");
                        ...
                        if (alfabeto_sostituzione[i] == alfabeto_sostituzione[b])
                          {
                            ...
                            if (caratteri_uguali > 1)
                              {
                                printf("\nL'alfabeto di criptazione inserito contiene dei caratteri uguali fra loro.\n");
