...
* - NON FUNZIONA QUANDO INSERISCO DEL TESTO CON COPIA/INCOLLA CHE HA UN RITORNO A CAPO (\n) <<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<
  ...
  switch (metodo)
    {
      case 1: //SE HO SCELTO CIFRARIO DI CESARE
        ...
        for (int i=0; stringa[i] != '\0'; i++) //se il messaggio è inferiore alla sua lunghezza massima
          {
